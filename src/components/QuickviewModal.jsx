...
const QuickviewModal = () => {
  ...
  useEffect(() => {
    ...
    // Listen for custom event from ProductTranding
  ...
  {selectedProduct ? (
    <>
      ...
      >
        {selectedProduct.images?.map((img, index) => (
          <div key={index}>
            <Link
              to={`/product-details/${selectedProduct.id}`}
              onClick={() => closeModal("quickview")}
            >
              <img src={img} className="img-fluid" alt={`p-${index}`} />
            </Link>
          </div>
        ))}
      </Slider>
    ...
    <p className="text-center py-4">Loading product...</p>
  )}
</div>
